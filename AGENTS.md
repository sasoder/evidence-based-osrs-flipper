...
This repo is a **harness, not an auto-trader.** The agent never places trades or plays the game;
it produces exact instructions the user executes manually. It is **invoked on demand** when the
user logs in and asks for a plan, not on a schedule — there is no autonomous cron. Read
`README.md` for setup. This file is the engine-agnostic contract and runtime workflow; engine
adapters (e.g. `.claude/skills/flip/SKILL.md` for Claude Code's `/flip` command) point here and
add only engine-specific input mechanics.
...
Parse the user's message first. If it already supplies liquid gp (and any preferences), skip
straight to step 1 — do not re-ask for anything already stated. Otherwise collect only the
missing inputs, in one round (a single structured multi-question prompt if the engine supports
one, otherwise one concise message):
...
- **Attendance**: `At keyboard — can manage offers (default)` → `intraday`; `Away 8h+ /
  sleeping` → `overnight`. This is what `--horizon` actually means: each lane owns its own
  timing (active hard-exits by 90m regardless), and overnight only exists to disable
...
empty plan — point that out and re-ask instead of running the planner.
...
`--horizon overnight` before sizing or writing intents; do not hand-filter short-horizon rows after
the fact.
